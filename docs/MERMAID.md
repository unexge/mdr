...
- Nested composite states with inherited or local directions and transitions
  to composites
- Concurrent composite regions separated by `--`, each with scoped start and
  end states
...
- Transitions across concurrent region boundaries
- Empty concurrent regions and multiline notes
...
and ER subgraphs are limited to 16 groups nested 8 levels deep. Composite
states allow up to 8 concurrent regions. All diagrams fall back when they
